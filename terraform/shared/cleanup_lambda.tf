...
# Note: The ZIP file is built by CI before terraform apply
resource "aws_lambda_function" "cleanup" {
  count = local.enable_cleanup_lambda ? 1 : 0

  filename         = "${path.module}/lambda/pr_cleanup.zip"
  source_code_hash = fileexists("${path.module}/lambda/pr_cleanup.zip") ? filebase64sha256("${path.module}/lambda/pr_cleanup.zip") : null
